...
async function enviarListaModems(telefono) {
  const modems = catalogo.filter((p) => p.id.startsWith("modem"));
  registrarMensaje(telefono, "bot", "[Envio la lista de modems]");
  await axios.post(
    GRAPH_URL,
    {
      messaging_product: "whatsapp",
      ...
      interactive: {
        type: "list",
        header: { type: "text", text: "Modems GalviusTech" },
        body: { text: "Elige el modem que mejor se ajusta a lo que necesitas:" },
        action: {
          button: "Ver modems",
          sections: [
            {
...
const deteccion = detectarProductoPorPalabraClave(texto);
  if (deteccion === "impresora-termica") {
    await manejarSeleccionProducto(telefono, "impresora-termica");
    return;
  }
  if (deteccion === "modem") {
    await enviarListaModems(telefono);
    return;
  }
