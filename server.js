...
function obtenerSesion(telefono) {
            if (!sesiones[telefono]) {
                          sesiones[telefono] = { paso: "inicio", pedido: {}, historial: [], transcripcion: [], pausado: false };
            }
...
app.get("/admin", requiereLogin, async (req, res) => {
          ...
          const filasClientes = clientes
                    .map(
                              (c) => `
                              <tr${c.pausado ? ' style="background:#fff3cd;"' : ""}>
                              ...
                              <td>${c.pausado ? "Pausado (bot no responde)" : "Activo"}</td>
                              <td><a href="/admin/pausa/${encodeURIComponent(c.telefono)}">${c.pausado ? "Reanudar bot" : "Pausar bot"}</a></td>
                              </tr>`
                              )
                    .join("");
...
app.get("/admin/chat/:telefono", requiereLogin, async (req, res) => {
          ...
          <a href="/admin/pausa/${encodeURIComponent(cliente.telefono)}">${cliente.pausado ? "Reanudar bot" : "Pausar bot y escribir yo misma"}</a>
          </div>
          <div class="chat">
